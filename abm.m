function [I,X,U] = abm(individualValues,network,populationSize,totalTime,networkAdj,initialAdopter,initialAware,initialUnaware,d,c,b,bb,k,P)
...
attributes = zeros(6,populationSize);
%%row1% current state
...
format long
%%% Transition %%%
if rand(1) < exp(-d*attributes(6,agent)) % likelihood of becoming a potential adopter [from the DE model]
...
format long
%%% Transition %%%
if rand(1) > exp(-d*attributes(6,agent)) % likelihood of finding the price unacceptable
